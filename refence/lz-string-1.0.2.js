...
var LZString = {
  ...
  writeBits : function(numBits, value, data) {
    ...
    for (var i=0 ; i<numBits ; i++) {
      LZString.writeBit(value&1, data);
      value = value >> 1;
    }
  },
  ...
  produceW : function (context) {
    if (Object.prototype.hasOwnProperty.call(context.dictionaryToCreate,context.w)) {
      if (context.w.charCodeAt(0)<256) {
        LZString.writeBits(context.numBits, 0, context.data);
        LZString.writeBits(8, context.w, context.data);
      } else {
        LZString.writeBits(context.numBits, 1, context.data);
        LZString.writeBits(16, context.w, context.data);
      }
      LZString.decrementEnlargeIn(context);
      delete context.dictionaryToCreate[context.w];
    } else {
      LZString.writeBits(context.numBits, context.dictionary[context.w], context.data);
    }
    LZString.decrementEnlargeIn(context);
  },
  ...
  compress: function (uncompressed) {
    ...
    LZString.writeBits(context.numBits, 2, context.data);
    
    // Flush the last char
    while (context.data.val>0) LZString.writeBit(0,context.data)
  ...
  readBits : function(numBits, data) {
    ...
    while (power!=maxpower) {
      res |= LZString.readBit(data) * power;
      power <<= 1;
    }
    return res;
  },
  ...
  decompress: function (compressed) {
    ...
    next = LZString.readBits(2, data);
    switch (next) {
      case 0: 
        c = String.fromCharCode(LZString.readBits(8, data));
        break;
      case 1: 
        c = String.fromCharCode(LZString.readBits(16, data));
        break;
      case 2: 
        return "";
    }
    ...
    w = result = c;
    while (true) {
      c = LZString.readBits(numBits, data);
